...
export default function Navbar({ state }: any) {
  ...
  return (
    <div className="w-full border-b bg-light">
      <MaxWidthWrapper>
        <nav className="relative flex h-20 w-full items-center justify-between gap-6">
          {/* Kiri: Logo & Menu */}
          <div className="flex items-center gap-10">
            <Logo />
            <ul className="hidden list-none space-x-8 text-davy-gray md:flex">
              {sub.map((item, index) => (
                <li key={index}>
                  <Link
                    href={item.path}
                    className={`bg-primary-gradient bg-clip-text text-base font-medium duration-300 hover:text-transparent ${
